...
function createWavefont (opts) {
	...
	// opts.dotSize = Math.max(opts.dotSize, opts.ratio*opts.levels);

	let spacePath = new opentype.Path();
	spacePath.moveTo(0, -opts.dotSize);
	spacePath.lineTo(width, -opts.dotSize);
	spacePath.lineTo(width, 0);
	spacePath.lineTo(0, 0);
